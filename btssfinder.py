...
'''
pathTemp = "/tmp/" #for temporary files generated by btssfinder
# useful function
#==============================================================================#
    ...
    for tss_pos in obj.TSSs[list_TSS].items():
        ...
        if tss_pos[0] - mp < 0:
            ...
            if obj.TSSs[list_TSS][pos].strand:
                    sequence = obj.seq[(len(obj.seq)-reste):len(obj.seq)] + obj.seq[0:(tss_pos[0]+1)]
            else:
                print "TSS n° "+str(tss_pos)+" has lost…"
        if tss_pos[0] + ma > len(obj.seq):
            print "TSS n° "+str(tss_pos)+" has lost…"
        if obj.TSSs[list_TSS][pos].strand:
    ...
    for line in my_file.readlines():
        if line[0]!= '#':
            if line != '\n':
                ...
                f.write(str(pos)+'\t'+line1[3]+'\t'+str(gene)+'\t'+sig+'\t'+str(b10PosRealLeft)+','+
str(b10PosRealRight)+','+str(b35PosRealLeft)+','+str(b35PosRealRight)+'\n')
    my_file.close()
    f.close()
