...
def draw_melting_energy_circle(melting_energy,name, *args, **kwargs):
    ...
    plt.annotate('Melting energy '+name, xy=(center_x,radius), xytext=(center_x,center_y),verticalalignment = 'center', horizontalalignment = 'center', wrap=True)
    fig.set_size_inches(width, height)
    plt.tight_layout()
    plt.savefig(basedir+"data/"+name+"/annotation/melting_energy.png", dpi=500, transparent=False)        
    plt.close()
...
def draw_expression_circles(gen, *arg, **kwargs):
    ...
    for cond in gen.genes_valid.keys():
        ...
        if form == "png":
            plt.savefig(path+"/circle-"+cond.replace("/","-")+".png", dpi=1000, transparent=False)
        elif form == "svg":
