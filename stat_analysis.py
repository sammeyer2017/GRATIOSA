...
'''
This module allows performing statistical analysis (enrichment test,
proportion test) of omics and spatial data.
Such data are:
+ genes' orientation attributed to a Genome instance
    using load_gene_orientation method
+ genes' differential expression attributed to a Genome instance using
    load_state_from_FC
+ genes' expression attributed to a Genome instance using
    load_genomic_expression method
+ GO annotation attributed to a Genome instance using load_GO method
+ HiC data attributed to a Genome instance using load_loops and load_borders
    methods
+ signals coverage attributed to a Genome instance using load_genomic_signal
    method
'''
import numpy as np
...
def data_classification(data_x, data_y, class_nb, *args, **kwargs):
    ...
    Required Args:
        ...
        data_y (list): list of data associated with each element such as signal
                        coverage or gene expression
        class_nb (int.): number of classes (fractions) to create

    Optional Args:
        class_names (list): list of names to give to each class
                            By default, each class will be named by a number
                            between 0 and class_nb.
        thresholds (list of ints.): Thresholds used for the classification.
                                    If not given, the classification will
                                    distribute the data in fractions of equal
                                    sizes.
    ...
    if len(class_names) != class_nb:
        ...
        print("WARNING: the number of class names isn't equal to the class_nb")
        print(f"WARNING: the number of class was set to {len(class_names)}")
    if thresholds:
    ...
    # Option 2: classification in equal-size fractions
    else:
...
def proportion_test(dict_cats,
    ...
    Optional args:
        ...
        alt_hyp ("two-sided" or "one-sided"): alternative hypothesis.
            ...
            p-value will be kept. See statsmodels documentation for more details.
            (default: "one-sided")
    ...
    # differents. Performs one-sided or two-sided proportions test depending on
    # the "alt_hyp" argument.
    comp = []
    pvals = {}
