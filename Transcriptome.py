...
class Transcriptome:
    ...
    def load_expression(self):
        ...
        [3] Expression column [4] is Log ? [5] Separator 
        ...
        Outputs: 
            self.genes[locus].expression (dict.): new attribute of Gene instances 
                                                  related to the Transcriptome 
                                                  instance given as argument.
                                                  Dictionary of shape {condition: 
                                                  expression level (float.)}
            self.genes_valid_fc (dict.): Dictionary of shape {condition: list of 
                                         genes having an expression value}
    ...
    def compute_fc_from_expr(self, ctrls, conds, condname):
        ...
        compute_fc_from_expr computes, for each gene:
            1 - the mean of expression values (such as log2rpkm) in a list of controls 
                replicates (ctrls)
            2 - the mean of expression values in a list of tests replicates (conds)
            3 - the fold-change ie the difference between the mean control and the mean 
                test values
        ...
        Outputs: 
            self.genes[locus].fc_pval[condname] (tuple): 
                    new attribute of Gene instances related to the Transcriptome 
                    instance given as argument.Tuple of shape (fold-change,pvalue).
            self.genes_valid_fc[condname] (list.): list of genes having a 
                                                   fold-change and a pvalue 
                                                   corresponding to the new condition
    ...
    def load_fc_pval(self):
        ''' 
        loads Fold-changes and p-values (if available) from data files that are 
        in the /fold_changes/ directory. 
        ...
        Outputs: 
            self.genes[locus].fc_pval (dict.): new attribute of Gene instances 
                                               ...
                                               Dictionary of shape {condition: 
                                               (fold-change,pvalue)}.
                                               If no p-value is available 
                                               in the data file, 0 is set 
                                               as p-value.
            self.genes_valid_fc (dict.): Dictionary of shape {condition: list of 
                                         genes having at least a fold-change}
    ...
    def compute_state_from_fc(self, thresh_pval=0.05,thresh_fc=0):
        ...
        Outputs:
            self.genes[locus].state (dict.): new attribute of Gene instances 
                                            related to the Transcriptome instance
                                            given as argument. Dictionary of shape 
                                            ...
                                            - 'null' if the gene is not present 
                                                in the data
            self.statesFC (dict. of dict.): new attribute of the Transcriptome instance.
                                            ...
                                            corresponding to each state ('act', 'rep',
                                             'non' or 'null').
            self.genes[locus].fc_pval (dict.): new attribute of Gene instances 
                                               related to the Transcriptome instance given 
    ...
    def load_rnaseq_cov(self,cond="all",compute_from_bam=False):
        ...
        either from:
            .npz files (described in a cov.info file and computed
                        when new data are loaded for the first time)
                        which are in the /rnaseq_cov/ directory,
            coverage files which are in the /rnaseq_cov/ directory 
                        and are described in cov_txt.info file,
            paired-end .bam reads files which are in the /rnaseq_reads/ directory 
                        and are treated with useful_functions_transcriptome.cov_
                        from_reads function.

        To use directly new coverages data, both forward and reverse coverage data
        files are required. These files contain the coverage for each genomic 
        position (one line = one genomic position, no position can be ommited). 
        The importation of these new data require a cov_txt.info file, in the 
        /rnaseq_cov/ directory, containing the following information : 
        [0] Condition  
        ...
        Args: 
            ...
            compute_from_bam (Boolean.): if True, computes, using useful_functions_
                                transcriptome.cov_from_reads, coverage from paired-end
                                .bam reads files that are, with a bam_files.info file, 
                                in the /rnaseq_reads/ directory. 
        ...
        Outputs:
            self.rnaseq_cov_pos: dictionary of shape {condition: cov+}
                                 ...
                                 per genomic position for the + strand (forward
                                 coverage)
        ...
        Example: 
            ...
            # To compute the coverage from a new .bam files obtained from paired-end data
            # for example with: tophat2 genome/MG1655 Test_1.fastq.gz Test_2.fastq.gz
        ...
        if os.path.exists(f"{path2dir}cov_txt.info"):
            ...
            with open(f"{path2dir}cov_txt.info","r") as f: 
                ...
                for line in f:
                    ...
                    # create .npy from .txt
                    rnaseq_cov_neg=np.loadtxt(path2dir+line[1], usecols=[int(line[4])], skiprows= int(line[3]))
                    rnaseq_cov_pos=np.loadtxt(path2dir+line[2], usecols=[int(line[4])], skiprows= int(line[3]))
                    # save .npy into .npz
                    np.savez(f"{path2dir}{line[0]}_cov.npz", cov_pos=rnaseq_cov_pos, cov_neg=rnaseq_cov_neg)
                    # update cov.info
            ...
            with open(f"{path2dir}cov.info","r") as f:
                ...
                for line in f: 
                    ...
                    if cond == ["all"] or line[0] in cond :
                        ...
                        # load attributes
                        self.rnaseq_cov_neg[line[0]]= np.load(path2dir+line[1])["cov_neg"]
                        self.rnaseq_cov_pos[line[0]]= np.load(path2dir+line[1])["cov_pos"]
                if cond != ["all"] :
    ...
    def load_cov_start_end(self,compute_from_bam = False):
        ...
        To compute new coverages data from paired-end .bam reads files, the data 
        files and a bam_files.info file have to be in the /rnaseq_reads/ directory 
        and the input argument "compute_from_bam" has to be set to "True".
        The bam_files.info files contains:
        [0] Condition [1] Reads filename 
        During the importation of these new data, a corresponding .npz files 
        is created and the cov_start_stop.info file automatically completes itself.
        For the next importations of these data, only the npz. and the 
        cov_start_stop.info will be loaded, thus allowing faster loading.

        Args: 
            self (Transcriptome instance)
            compute_from_bam(Boolean): 
                                if True, computes, using useful_functions_
                                transcriptome.cov_start_stop_from_reads, 
                                coverage from paired-end
                                .bam reads files that are, with a bam_files.info 
                                file, in the /rnaseq_reads/ directory. 
                                /!\ Works only with paired-end files /!\
        Outputs:
        ...
        if os.path.exists(f"{path2dir}cov_start_stop.info"): 
            with open(f"{path2dir}cov_start_stop.info","r") as f:
                ...
                for line in f: # for each condition
                    ...
                    self.cov_end[line[0]] = {}
                    self.cov_start[line[0]][0] = np.load(path2dir+line[1])["cov_start_neg"]
                    self.cov_start[line[0]][1] = np.load(path2dir+line[1])["cov_start_pos"]
                    self.cov_end[line[0]][0] = np.load(path2dir+line[1])["cov_end_neg"]
                    self.cov_end[line[0]][1] = np.load(path2dir+line[1])["cov_end_pos"]
            f.close()
    ...
    def compute_log2rpkm_from_cov(self, cond='all',before=100):
        ...
        4 - adds the new .csv file informations in the expression.info file in 
            the /expression/ directory
        ...
        Args:
            ...
            cond (list of str.): 
                    ...
                    converted. All selected conditions have to be list in 
        ...
        Outputs: 
            self.genes[locus].rpkm (dict.): 
                    new attribute of Gene instances related to the Transcriptome 
                    instance given as argument.
                    Dictionary of shape {condition: RPKM value}, containing the
                    RPKM for each computed condition.
                ...
                try:
                    ...
                    # gene in + strand
                        for c in cond2convert: # for each condition of cov
                            self.genes[g].add_single_rpkm(c, np.mean(self.rnaseq_cov_pos[c][(self.genes[g].left-before):self.genes[g].right]), np.sum(self.rnaseq_cov_pos[c])+np.sum(self.rnaseq_cov_neg[c]))
                    elif not self.genes[g].strand:
                    # gene in - strand
                        for c in cond2convert:
                            self.genes[g].add_single_rpkm(c, np.mean(self.rnaseq_cov_neg[c][self.genes[g].left:(self.genes[g].right+before)]), np.sum(self.rnaseq_cov_pos[c])+np.sum(self.rnaseq_cov_neg[c]))
                except Exception as e:
                    print(f'WARNING: {g} , {cond}, {e}')
                    pass

        # Saving log2RPKM in a .csv file and completing the expression.info file
            file = []
